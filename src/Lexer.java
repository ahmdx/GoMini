			...
			if (YY_F != yy_next_state) {
				...
				if (YY_NO_STATE == yy_last_accept_state) {
					...
					case -7:
						break;
					case 7:
						{ stackOfOpenCurlyBraces.add(yyline); return new Symbol(sym.OPEN_CURLY, yytext()); }
					case -8:
						break;
					case 8:
						{ if(!stackOfOpenParans.isEmpty()) { openParansNotClosed = true; return new Symbol(sym.ERROR, "You have a missing bracket in line " + (yyline+1)); } if(!stackOfOpenCurlyBraces.isEmpty()) { stackOfOpenCurlyBraces.pop(); return new Symbol(sym.CLOSE_CURLY, yytext()); } else return new Symbol(sym.ERROR, "} has no matching { in line " + (yyline+1)); }
					case -9:
						break;
					case 9:
						{ stackOfOpenParans.add(yyline); return new Symbol(sym.OPEN_PARAN, yytext()); }
					case -10:
						break;
					case 10:
						{if(!stackOfOpenParans.isEmpty()) { stackOfOpenParans.pop(); return new Symbol(sym.CLOSE_PARAN, yytext()); } else return new Symbol(sym.ERROR, ") has no matching ( in line " + (yyline+1)); }
					case -11:
						break;
					case 11:
						{ stackOfOpenSquares.add(yyline); return new Symbol(sym.OPEN_SQUARE, yytext()); }
					case -12:
						break;
					case 12:
						{ if(!stackOfOpenSquares.isEmpty()) { stackOfOpenSquares.pop(); return new Symbol(sym.CLOSE_SQUARE, yytext()); } else return new Symbol(sym.ERROR, "] has no matching [ in line " + (yyline+1));}
					case -13:
						break;
					case 13:
